...
%     confusionchart(SS_ACCmatrix,'Title',['subj ',subjdir,' single sample acc'],'Normalization','row-normalized')
    ...
    for iTH=1:8
        ...
        T_ACCmatrix(:,:,iTH)=confusionmat(Ck(timeForDecision>0),trialDecision(timeForDecision>0));
        AVGtime(iTH)=mean(timeForDecision(timeForDecision>0));
        NUMdecisions(iTH)=sum(timeForDecision>0);
    end
...
%     xline(winmasked(diff(Tmasked)>0))
    ...
    subplot(132)
    hold on
    plot(th,NUMdecisions/T0,'B')
    plot(th,squeeze(T_ACCmatrix(1,1,:)+T_ACCmatrix(2,2,:))/T0,'r')
    plot(th,squeeze(T_ACCmatrix(1,1,:)+T_ACCmatrix(2,2,:))./squeeze(sum(T_ACCmatrix,[1,2])),'g')
    ...
    subplot(133)
    hold on
    plot(th,squeeze(T_ACCmatrix(1,1,:))./squeeze(T_ACCmatrix(1,1,:)+T_ACCmatrix(1,2,:)),'r')
    plot(th,squeeze(T_ACCmatrix(2,2,:))./squeeze(T_ACCmatrix(2,2,:)+T_ACCmatrix(2,1,:)),'b')
