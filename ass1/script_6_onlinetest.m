...
for subject=1:length(SUBJ)
    ...
    SS_ACCmatrix(:,:,subject)=confusionmat(groundTruth,Gk);

    figure
    subplot(121)
    confusionchart(SS_ACCmatrix(:,:,subject),'Title',['subj ',subjdir,' ONLINE single sample acc'],'Normalization','row-normalized')
    ...
    timeForDecisionONLINE=zeros(trials,1);
    ...
    for iW=2:size(pp,1)
        ...
        if Tmasked(iW-1)~=T0 %all'inizio di ogni trial resetto il valore di D
            ...
            if sum(D(iW-1,:)<th)==2 %control if the decisionboundary has been crossed
                ...
                D(iW,:) = D(iW-1,:);
                if timeForDecisionONLINE(T0)==0
                    timeForDecisionONLINE(T0)=0.0625*time0;
                end 
            end
        end
    end
    ...
    confusionchart(T_ACCmatrix(:,:,subject),'Title',['subj ',subjdir,' ONLINE trial acc'],'Normalization','row-normalized')
    ...
    T_ACC_ONLINEmatrix=T_ACCmatrix(:,:,subject);
    SS_ACC_ONLINEmatrix=SS_ACCmatrix(:,:,subject);

    %%
    save(fullfile('results','ACC',subjdir, 'ONLINE_ACC.mat'),'T_ACC_ONLINEmatrix','SS_ACC_ONLINEmatrix','timeForDecisionONLINE')
    pause
