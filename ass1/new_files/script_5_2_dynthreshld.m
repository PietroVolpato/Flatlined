...
for subject=1:length(SUBJ)
    ...
    [SS_ACC_DYNOFFLINEmatrix,classes]=confusionmat(groundTruth,Gk);
    ...
    timeForDecision=zeros(trials,length(th_vec));
    for iTH=1:length(th_vec)
        ...
        for iW=2:size(pp,1)
            ...
            if Tmasked(iW-1)~=T0 %all'inizio di ogni trial resetto il valore di D
                ...
                D(iW,1) = dynupdate(D(iW-1,1),pp(iW,1),params);
                D(iW,2) = dynupdate(D(iW-1,2),pp(iW,2),params);
                else 
                    D(iW,:) = D(iW-1,:);
    ...
    %%
    SUBJ_DYN_TH=[0.65 0.70 0.65 0.70 0.65 0.70 nan 0.70];
    ...
    ylim([0,1])
    yline((SS_ACC_DYNOFFLINEmatrix(1,1)+SS_ACC_DYNOFFLINEmatrix(2,2))/sum(SS_ACC_DYNOFFLINEmatrix,'all'),'-.g')
    xline(SUBJ_DYN_TH(subject),'k')
    legend({'fract of decisions','fract of correct dec','dynamical model accuracy','sing samp acc','chosen_threshold'},'Location','south')
    ...
    yline(SS_ACC_DYNOFFLINEmatrix(1,1)/(SS_ACC_DYNOFFLINEmatrix(1,1)+SS_ACC_DYNOFFLINEmatrix(1,2)),'-.r')
    yline(SS_ACC_DYNOFFLINEmatrix(2,2)/(SS_ACC_DYNOFFLINEmatrix(2,2)+SS_ACC_DYNOFFLINEmatrix(2,1)),'-.b')
    xline(SUBJ_DYN_TH(subject),'k')
    ylim([0,1])
    legend({['class' num2str(classes(1)) ' acc'],['class' num2str(classes(2)) ' acc'],['sing samp ' num2str(classes(1)) ' acc'],['sing samp' num2str(classes(2)) ' acc'],'chosen threshold'},'Location','south')
    title(['subj ',subjdir,' OFFLINE'])
    xlabel('threshold value []')
    grid on
    ...
    figure
    subplot(121)
    confusionchart(SS_ACC_DYNOFFLINEmatrix,classes,'Title',['subj ',subjdir,' OFFLINE single sample acc'],'Normalization','row-normalized')

    subjITH=abs(th_vec-SUBJ_DYN_TH(subject))<eps;
    T_ACC_DYNOFFLINEmatrix=Trial_ACCmatrix(:,:,subjITH);
    timeForDecisionDYNOFFLINE=timeForDecision(:,subjITH);

    subplot(122)
    confusionchart(T_ACC_DYNOFFLINEmatrix,classes,'Title',['subj ',subjdir,' OFFLINE trial acc'],'Normalization','row-normalized')

    mkdir(fullfile('results','ACC',subjdir))
    save(fullfile('results','ACC',subjdir, 'OFFLINE_DYNACC.mat'),'T_ACC_DYNOFFLINEmatrix','SS_ACC_DYNOFFLINEmatrix','timeForDecisionDYNOFFLINE')
    pause
end

save(fullfile('results','thresholds_dynamical'),'SUBJ_DYN_TH')
