...
%% apply exponential smoothing
    ...
    xline(chosen_alpha,'k')
    ylim([0,1])
    legend({['class' num2str(classes(1)) ' acc'],['class' num2str(classes(2)) ' acc'],['sing samp ' num2str(classes(1)) ' acc'],['sing samp' num2str(classes(2)) ' acc'],'optimal \alpha'},'Location','south')
    title(['subj ',subjdir,' OFFLINE'])
    xlabel('decay rate []') 
    grid on
