...
for subject=1:length(SUBJ)
    ...
    [SS_ACC_EXPOFFLINEmatrix,classes]=confusionmat(groundTruth,Gk);
    ...
    ylim([0,1])
    yline((SS_ACC_EXPOFFLINEmatrix(1,1)+SS_ACC_EXPOFFLINEmatrix(2,2))/sum(SS_ACC_EXPOFFLINEmatrix,'all'),'-g')
    xline(SUBJ_TH(subject),'k')
    ...
    yline(SS_ACC_EXPOFFLINEmatrix(1,1)/(SS_ACC_EXPOFFLINEmatrix(1,1)+SS_ACC_EXPOFFLINEmatrix(1,2)),'-.r')
    yline(SS_ACC_EXPOFFLINEmatrix(2,2)/(SS_ACC_EXPOFFLINEmatrix(2,2)+SS_ACC_EXPOFFLINEmatrix(2,1)),'-.b')
    xline(SUBJ_TH(subject),'k')
    ylim([0,1])
    ...
    figure
    subplot(121)
    confusionchart(SS_ACC_EXPOFFLINEmatrix,classes,'Title',['subj ',subjdir,' OFFLINE single sample acc'],'Normalization','row-normalized')

    subjITH=abs(th_vec-SUBJ_TH(subject))<eps;
    T_ACC_EXPOFFLINEmatrix=Trial_ACCmatrix(:,:,subjITH);
    timeForDecisionEXPOFFLINE=timeForDecision(:,subjITH);
    subplot(122)
    confusionchart(T_ACC_EXPOFFLINEmatrix,classes,'Title',['subj ',subjdir,' OFFLINE trial acc'],'Normalization','row-normalized')

    mkdir(fullfile('results','ACC',subjdir))
    save(fullfile('results','ACC',subjdir, 'OFFLINE_EXPACC.mat'),'T_ACC_EXPOFFLINEmatrix','SS_ACC_EXPOFFLINEmatrix','timeForDecisionEXPOFFLINE')
    pause
end
