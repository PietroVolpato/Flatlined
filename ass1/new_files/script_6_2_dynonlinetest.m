...
load("results\thresholds_dynamical.mat")
...
for subject=1:length(SUBJ)
    ...
    %chi and th value derived by previous analyses
    params.chi=0.06;
    th=SUBJ_DYN_TH(subject);
    ...
    timeForDecisionDYNONLINE=zeros(trials,1);
    ...
    for iW=2:size(pp,1)
        ...
        if Tmasked(iW-1)~=T0 %all'inizio di ogni trial resetto il valore di D
            ...
            if sum(D(iW-1,:)<th)==2 %control if the decisionboundary has been crossed
                ...
                D(iW,:) = D(iW-1,:);
                if timeForDecisionDYNONLINE(T0)==0
                    timeForDecisionDYNONLINE(T0)=0.0625*time0;
                end 
            end
        end
    end
    ...
    yline(0.5,'-r')
    title(['sub ',subjdir,' dynamically smoothed decision'])
    xlabel('time [s]')
    ylabel('D(time) []')
    ylim([0,1])
    ...
    T_ACC_DYNONLINEmatrix=T_ACCmatrix(:,:,subject);
    SS_ACC_DYNONLINEmatrix=SS_ACCmatrix(:,:,subject);

    %%
    save(fullfile('results','ACC',subjdir, 'ONLINE_ACC_dynsys.mat'),'T_ACC_DYNONLINEmatrix','SS_ACC_DYNONLINEmatrix','timeForDecisionDYNONLINE')
    pause
