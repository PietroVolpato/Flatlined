...
%% apply exponential smoothing
    ...
    subplot(132)
    hold on
    plot(chi,NUMdecisions/T0,'B')
    plot(chi,squeeze(T_ACCmatrix(1,1,:)+T_ACCmatrix(2,2,:))/T0,'r')
    plot(chi,squeeze(T_ACCmatrix(1,1,:)+T_ACCmatrix(2,2,:))./squeeze(sum(T_ACCmatrix,[1,2])),'g')
    yline((SS_ACCmatrix(1,1)+SS_ACCmatrix(2,2))/sum(SS_ACCmatrix,'all'),'.-g')
    xline(chosen_alpha,'k')
    ylim([0,1])
    legend({'fract of decisions','fract of correct dec','model accuracy','sing samp acc','optimal \alpha'},'Location','south')
    ...
    subplot(133)
    hold on
    plot(chi,squeeze(T_ACCmatrix(1,1,:))./squeeze(T_ACCmatrix(1,1,:)+T_ACCmatrix(1,2,:)),'r')
    plot(chi,squeeze(T_ACCmatrix(2,2,:))./squeeze(T_ACCmatrix(2,2,:)+T_ACCmatrix(2,1,:)),'b')
    yline(SS_ACCmatrix(1,1)/(SS_ACCmatrix(1,1)+SS_ACCmatrix(1,2)),'-.r')
    yline(SS_ACCmatrix(2,2)/(SS_ACCmatrix(2,2)+SS_ACCmatrix(2,1)),'-.b')
    xline(chosen_alpha,'k')
    ylim([0,1])
    legend({['class' num2str(classes(1)) ' acc'],['class' num2str(classes(2)) ' acc'],['sing samp ' num2str(classes(1)) ' acc'],['sing samp' num2str(classes(2)) ' acc'],'optimal \chi'},'Location','south')
    title(['subj ',subjdir,' OFFLINE'])
    xlabel('chi []') 
    grid on
