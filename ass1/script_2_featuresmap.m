%% THIS SCRIPT LOADS ALL THE .mat OFFLINE FILES COMPITES THE FEATURES MAP AND
...
for subject=1:length(SUBJ)
    ...
    subplot(1,2,1)
    imagesc(prod(FSscore2d,3)',[0 1])
    title(['prod subject ',subjdir])
    xlabel('frequencies')
    ylabel('channel')

    subplot(1,2,2 )
    imagesc(sum(FSscore2d,3)',[0 3])
