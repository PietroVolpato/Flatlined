...
%% apply exponential smoothing
    ...
    for iChi=1:10
        ...
        [T_ACCmatrix(:,:,iChi),classes]=confusionmat(Ck(timeForDecision>0),trialDecision(timeForDecision>0));
        AVGtime(iChi)=mean(timeForDecision(timeForDecision>0));
        NUMdecisions(iChi)=sum(timeForDecision>0);
    end
    ...
    subplot(132)
    hold on
    plot(chi,NUMdecisions/T0,'B','LineWidth',2)
    plot(chi,squeeze(T_ACCmatrix(1,1,:)+T_ACCmatrix(2,2,:))/T0,'r','LineWidth',2)
    plot(chi,squeeze(T_ACCmatrix(1,1,:)+T_ACCmatrix(2,2,:))./squeeze(sum(T_ACCmatrix,[1,2])),'g','LineWidth',2)
    yline((SS_ACCmatrix(1,1)+SS_ACCmatrix(2,2))/sum(SS_ACCmatrix,'all'),'-.g','LineWidth',2)
    xline(chosen_alpha,'k','LineWidth',2)
    ylim([0,1])
    legend({'fract of decisions','fract of correct dec','model accuracy','sing samp acc','optimal \chi'},'Location','south','FontSize',9)
    ...
    subplot(133)
    hold on
    plot(chi,squeeze(T_ACCmatrix(1,1,:))./squeeze(T_ACCmatrix(1,1,:)+T_ACCmatrix(1,2,:)),'r','LineWidth',2)
    plot(chi,squeeze(T_ACCmatrix(2,2,:))./squeeze(T_ACCmatrix(2,2,:)+T_ACCmatrix(2,1,:)),'b','LineWidth',2)
    ...
    xline(chosen_alpha,'k','LineWidth',2)
    ylim([0,1])
    legend({['class' num2str(classes(1)) ' acc'],['class' num2str(classes(2)) ' acc'],['sing samp ' num2str(classes(1)) ' acc'],['sing samp' num2str(classes(2)) ' acc'],'optimal \chi'},'Location','south','FontSize',9)
    title(['subj ',subjdir,' OFFLINE'])
    xlabel('chi []') 
    grid on
